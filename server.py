...
SHAREHOLDER_GIFT_CURRENT_YEAR = datetime.now().year
SHAREHOLDER_GIFT_WESPAI_URL = f"https://stock.wespai.com/stock{SHAREHOLDER_GIFT_CURRENT_YEAR - 1911}"
SHAREHOLDER_GIFT_IDEAL_URL = "https://souvenir.ideal-labs.com/"
SHAREHOLDER_GIFT_HONSEC_URL = "https://srd.honsec.com.tw/stock/souvenir.aspx"
SHAREHOLDER_GIFT_CACHE_TTL = int(os.environ.get("SHAREHOLDER_GIFT_CACHE_TTL", "1800"))
SHAREHOLDER_GIFT_CACHE: dict[str, tuple[float, Any]] = {}
...
def cached_value(name: str, loader) -> Any:
    now = time.time()
    item = SHAREHOLDER_GIFT_CACHE.get(name)
    if item and now - item[0] < SHAREHOLDER_GIFT_CACHE_TTL:
        return item[1]
    value = loader()
    SHAREHOLDER_GIFT_CACHE[name] = (now, value)
    return value


def shareholder_parse_mmdd_to_iso(value: str) -> str | None:
    cleaned = normalize_text(value)
    match = re.fullmatch(r"(\d{2})\.(\d{2})", cleaned)
    if not match:
        return None
    month, day = match.groups()
    return f"{SHAREHOLDER_GIFT_CURRENT_YEAR}-{month}-{day}"


def shareholder_parse_roc_date(value: str) -> str | None:
    cleaned = normalize_text(value)
...
def shareholder_parse_roc_range(value: str) -> tuple[str | None, str | None]:
    cleaned = normalize_text(value)
    if cleaned in {"", "-"}:
        return None, None
    match = re.fullmatch(r"(\d{2,3}/\d{2}/\d{2})\s*[-~]\s*(\d{2,3}/\d{2}/\d{2})", cleaned)
    if not match:
        single = shareholder_parse_roc_date(cleaned)
        return single, single
    return shareholder_parse_roc_date(match.group(1)), shareholder_parse_roc_date(match.group(2))


def shareholder_load_wespai() -> dict[str, dict[str, Any]]:
    html = fetch_text(SHAREHOLDER_GIFT_WESPAI_URL)
    parser = TableParser()
    parser.feed(html)
    target_table: list[list[Cell]] | None = None
    ...
    if not target_table or len(target_table) < 2:
        raise ValueError("找不到撿股讚股東會紀念品表格")
    ...
    for row in target_table[1:]:
        ...
        rows[code] = {
            ...
            "meeting_date": shareholder_parse_mmdd_to_iso(values[6]),
            "meeting_city": values[7],
            "last_buy_date": shareholder_parse_mmdd_to_iso(values[8]),
            "transfer_agent_short": values[9],
            "transfer_agent_phone": values[10],
            "odd_lot_mail": values[12],
            "reelection": values[13],
            "source_url": SHAREHOLDER_GIFT_WESPAI_URL,
            "official_doc_url": (
                f"https://doc.twse.com.tw/server-java/t57sb01?step=1&colorchg=1&co_id={code}"
                f"&year={SHAREHOLDER_GIFT_CURRENT_YEAR - 1911}&mtype=F&"
            ),
        }
    return rows


def shareholder_load_ideal() -> dict[str, dict[str, Any]]:
    html = fetch_text(SHAREHOLDER_GIFT_IDEAL_URL)
...
def shareholder_is_company_start(tokens: list[str], index: int) -> bool:
    return index + 2 < len(tokens) and tokens[index + 1].endswith("年") and tokens[index + 2].startswith("代號 ")


def shareholder_load_honsec() -> dict[str, dict[str, Any]]:
    html = fetch_text(SHAREHOLDER_GIFT_HONSEC_URL)
    ...
    while index < len(tokens):
        if not shareholder_is_company_start(tokens, index):
            index += 1
            continue
        company_name = tokens[index]
        code = tokens[index + 2].replace("代號", "").strip()
        index += 3
        record: dict[str, Any] = {"code": code, "company_name": company_name, "source_url": SHAREHOLDER_GIFT_HONSEC_URL}
        for field_name, label in schema:
            if index >= len(tokens) or tokens[index] != label:
                record[field_name] = ""
                continue
            index += 1
            if index >= len(tokens) or tokens[index] in known_labels or shareholder_is_company_start(tokens, index):
                record[field_name] = ""
            else:
                record[field_name] = tokens[index]
                index += 1
        record["last_buy_date"] = shareholder_parse_roc_date(record["last_buy_date_text"])
        record["evote_start_date"], record["evote_end_date"] = shareholder_parse_roc_range(record["evote_period_text"])
        record["evote_pickup_start_date"], record["evote_pickup_end_date"] = shareholder_parse_roc_range(
...
def shareholder_source_bundle() -> dict[str, Any]:
    return {
        "wespai": cached_value("shareholder-wespai", shareholder_load_wespai),
        "ideal": cached_value("shareholder-ideal", shareholder_load_ideal),
        "honsec": cached_value("shareholder-honsec", shareholder_load_honsec),
    }


def shareholder_clean_codes(raw: str) -> list[str]:
    seen: set[str] = set()
    out: list[str] = []
...
def shareholder_build_record(code: str, sources: dict[str, Any]) -> dict[str, Any]:
    ...
    status = "partial" if (not is_published and company_name) else ("published" if is_published else "unpublished")
    source_links: list[dict[str, str]] = []
    if wespai:
        source_links.extend(
            [
                {"label": "撿股讚", "url": wespai["source_url"]},
                {"label": "官方開會資料", "url": wespai["official_doc_url"]},
            ]
        )
    if ideal:
        source_links.append({"label": "股東禮簿", "url": SHAREHOLDER_GIFT_IDEAL_URL})
    if honsec:
        source_links.append({"label": "宏遠股代", "url": SHAREHOLDER_GIFT_HONSEC_URL})
    return {
        "code": code,
        "companyName": company_name,
        "status": status,
        "isPublished": is_published,
        "souvenirName": (wespai or {}).get("souvenir_name") or (ideal or {}).get("souvenir_name") or (honsec or {}).get("souvenir_name") or "",
        "meetingDate": (ideal or {}).get("meeting_date") or (wespai or {}).get("meeting_date"),
        "lastBuyDate": (ideal or {}).get("last_buy_date") or (wespai or {}).get("last_buy_date") or (honsec or {}).get("last_buy_date"),
        "meetingCity": (wespai or {}).get("meeting_city", ""),
        "priceText": (wespai or {}).get("price_text", ""),
        "transferAgentName": (ideal or {}).get("transfer_agent_name") or (wespai or {}).get("transfer_agent_short") or "",
        "transferAgentPhone": (ideal or {}).get("transfer_agent_phone") or (wespai or {}).get("transfer_agent_phone") or "",
        ...
        "evoteStartDate": (ideal or {}).get("evote_start_date") or (honsec or {}).get("evote_start_date"),
        "evoteEndDate": (ideal or {}).get("evote_end_date") or (honsec or {}).get("evote_end_date"),
        "evotePickupStartDate": (honsec or {}).get("evote_pickup_start_date"),
        "evotePickupEndDate": (honsec or {}).get("evote_pickup_end_date"),
        ...
        "notes": "目前在整合來源中尚未看到今年紀念品公告，建議保留 watchlist 持續追蹤。" if status == "unpublished" else "",
...
class Handler(SimpleHTTPRequestHandler):
    def do_HEAD(self) -> None:
        ...
        if parsed.path == "/api/shareholder-gifts/lookup":
        ...
        if parsed.path == "/api/report.pdf":
            ...
            self.end_headers()
            return
        if parsed.path == "/shareholder-gift-tracker":
            self.path = "/shareholder-gift-tracker/index.html"
    ...
    def do_GET(self) -> None:
        parsed = urllib.parse.urlparse(self.path)
        if parsed.path == "/api/shareholder-gifts/lookup":
            self.serve_shareholder_gifts(parsed.query)
            return
        if parsed.path == "/api/report":
    ...
    def serve_shareholder_gifts(self, query: str) -> None:
        try:
            params = urllib.parse.parse_qs(query)
            raw_codes = params.get("codes", [""])[0]
            codes = shareholder_clean_codes(raw_codes)
            if not codes:
                body = json.dumps(
                    {"ok": False, "error": "請輸入至少一筆股票代號。", "results": []},
                    ensure_ascii=False,
                ).encode("utf-8")
            ...
            sources = shareholder_source_bundle()
            payload = {
                "ok": True,
                "generatedAt": datetime.now(TW_TZ).strftime("%Y-%m-%dT%H:%M:%S%z"),
                ...
                "results": [shareholder_build_record(code, sources) for code in codes],
            }
            ...
            error = json.dumps({"ok": False, "error": f"資料抓取失敗：{exc}", "results": []}, ensure_ascii=False).encode("utf-8")
